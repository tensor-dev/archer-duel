(function(){
    ...
    Room.prototype._switchPlayer = function() {
        ...
        var dir = Math.random(), windValue = Math.random() * 10;
    ...
    Room.prototype.addClient = function(socket, data) {
        ...
        if(player) {
            ...
            if(this._readyCount == 2) {
                ...
                broadcast.emit('gameStart', this._getState());
            }

            // broadcast fire to all players
            socket.on('fire', function(data){
                console.log("Received >fire< event for player " + player.toJSON().displayName + "'s socket");
                console.log("Player0: " + self._players[0].toJSON().displayName);
                console.log("Player1: " + self._players[1].toJSON().displayName);
                console.log("Current player number is " + self._currentPlayer);
            ...
            // collect 2 hit, apply it if to same player, switch player (will emit state change)
            ...
            socket.on('stopGame', function(data){
                console.log("Got >stopgame<");
                if(self._eventSync.stopGame) {
                    console.log("Got second >stopgame< with winner " + data.winner);
                    if(self._eventSync.stopGame == data.winner) {
                        ...
                        console.log("Room " + self._id + " got >stopGame< with different winners!");
                    }
                    self._eventSync.stopGame = 0;
                } else {
                    console.log("Got first >stopgame< with winner = " + data.winner);
